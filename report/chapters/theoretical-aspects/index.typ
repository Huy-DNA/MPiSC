...
#proof[
  ...
  _A read-front would return `false` when the queue is empty_: To put more precisely, for a read-front $r$, if by $r$'s linearization point, every successful `spsc_enqueue` $e'$ such that $e' < r$ has been matched by $d'$ such that $d' < d$, then $r$ would return `false`. That means any unmatched successful `spsc_enqueue` $e$ must have executed its linearization point at @line-spsc-enqueue-increment-last after $r$'s, or `First = Tail` before $r$'s linearization point
  - For an enqueuer's read-front, if $r$ doesn't pass @line-spsc-e-readFront-diff-cache-once, the statement holds. If $r$ passes @line-spsc-e-readFront-diff-cache-once, by the assumption, $r$ would execute @line-spsc-e-readFront-empty-twice, because $r$ sees that `First = Tail`.
  - For an dequeuer's read-front, $r$ must enter line @line-spsc-d-readFront-resync-last because `First_buf = Tail_buf`, due to from the dequeuer's point of view, `First_buf = First` and `Last_buf <= Last`. Similarly, $r$ must execute @line-spsc-d-readFront-empty and return `false`.
  ...
  - For a dequeuer's read-front, $r$ must not execute line @line-spsc-d-readFront-resync-last - @line-spsc-d-readFront-empty and execute @line-spsc-d-readFront-read instead. It's trivial that $r$ reads out the same value as the first dequeue $d$ such that $r < d$ because there can only be one dequeuer.
