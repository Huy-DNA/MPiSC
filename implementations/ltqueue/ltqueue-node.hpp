...
template <typename T> class LTNodeEnqueuer {
private:
...
public:
  LTNodeEnqueuer(MPI_Aint capacity_per_node, MPI_Aint dequeuer_rank,
  ...
  LTNodeEnqueuer(const LTNodeEnqueuer &) = delete;
  LTNodeEnqueuer &operator=(const LTNodeEnqueuer &) = delete;
...
template <typename T> class LTNodeDequeuer {
private:
  struct alignas(8) tree_node_t {
    int32_t rank;
    uint32_t tag;
...
public:
  LTNodeDequeuer(MPI_Aint capacity_per_node, MPI_Aint dequeuer_rank,
  ...
  LTNodeDequeuer(const LTNodeDequeuer &) = delete;
  LTNodeDequeuer &operator=(const LTNodeDequeuer &) = delete;
  ~LTNodeDequeuer() {
    MPI_Win_unlock_all(this->_min_timestamp_win);
    MPI_Win_unlock_all(this->_tree_win);
    MPI_Win_free(&this->_tree_win);
