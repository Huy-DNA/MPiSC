...
template <typename T> class UnboundedLTEnqueuer {
private:
...
public:
  UnboundedLTEnqueuer(MPI_Aint dequeuer_rank, MPI_Aint self_rank, MPI_Comm comm)
  ...
  UnboundedLTEnqueuer(const UnboundedLTEnqueuer &) = delete;
  UnboundedLTEnqueuer &operator=(const UnboundedLTEnqueuer &) = delete;
...
template <typename T> class UnboundedLTDequeuer {
private:
  struct alignas(8) tree_node_t {
    int32_t rank;
    uint32_t tag;
...
public:
  UnboundedLTDequeuer(MPI_Aint dequeuer_rank, MPI_Aint self_rank, MPI_Comm comm,
  ...
  UnboundedLTDequeuer(const UnboundedLTDequeuer &) = delete;
  UnboundedLTDequeuer &operator=(const UnboundedLTDequeuer &) = delete;
  ~UnboundedLTDequeuer() {
    MPI_Win_unlock_all(this->_min_timestamp_win);
    MPI_Win_unlock_all(this->_tree_win);
    MPI_Win_free(&this->_tree_win);
