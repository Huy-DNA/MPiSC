...
#proof[
  ...
  - Items are `dequeue`d in the order they are `enqueue`d: Suppose there are two `enqueue`s $e_1$, $e_2$ such that $e_1 arrow.double e_2$ and suppose they match $d_1$ $d_2$. Then we have obtained $e_1 arrow.double e_2$ either because:
    ...
    - $(1)$ applies, then $e_1$ precedes $e_2$, by @enqueue-enqueue-theorem, $d_1$ must precede $d_2$ or $d_1 arrow.double d_2$.
    Therefore, $e_1 arrow.double e_2 => d_1 arrow.double d_2$.
  - An item can only be `dequeue`d after it's `enqueue`d: Suppose there is an `enqueue` $e$ matched by $d$. By $(2)$, obviously $e =>$#sub($H'$)$d$.
  - If the queue is empty, `dequeue`s return nothing. Suppose a dequeue $d$ such that any $e arrow.double d$ is all matched by some $d'$ and $d' arrow.double d$, we will prove that $d$ is unmatched. By @matching-dequeue-enqueue-lemma, $d$ can only match an `enqueue` $e_0$ that precedes or overlaps with $d$.
    - If $e_0$ precedes $d$, by our assumption, it's already matched by another `dequeue`.
    - If $e_0$ overlaps with $d$, by our assumption, $d arrow.double e_0$. Then, we can only obtain this because $(4)$ applies, but then $d$ does not match $e_0$.
    Therefore, $d$ is unmatched.

  In conclusion, $=>$#sub($H'$) is a way we can order method calls in $H'$ sequentially that conforms to FIFO semantics. Therefore, we can also order method calls in $H$ sequentially that conforms to FIFO semantics as we only append `dequeue`s sequntially to the end of $H$ to obtain $H'$.
