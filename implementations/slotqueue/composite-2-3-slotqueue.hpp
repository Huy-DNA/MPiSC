...
#include "../lib/distributed-counters/composite-2-3-faa.hpp"
...
template <typename T> class Slot2C3Queue {
private:
  typedef uint64_t timestamp_t;
  constexpr static uint64_t MAX_TIMESTAMP = ~((uint64_t)0);
...
public:
  Slot2C3Queue(MPI_Aint capacity_per_node, MPI_Aint dequeuer_rank,
               MPI_Comm comm)
  ...
  Slot2C3Queue(const Slot2C3Queue &) = delete;
  Slot2C3Queue &operator=(const Slot2C3Queue &) = delete;

  Slot2C3Queue(Slot2C3Queue &&other) noexcept
