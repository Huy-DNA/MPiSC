...
template <typename T> class UnboundedSlotEnqueuer {
private:
  ...
  UnboundedSpscEnqueuer<data_t> _spsc;
...
public:
  UnboundedSlotEnqueuer(MPI_Aint dequeuer_rank, MPI_Aint self_rank,
                        MPI_Comm comm)
  ...
  UnboundedSlotEnqueuer(const UnboundedSlotEnqueuer &) = delete;
  UnboundedSlotEnqueuer &operator=(const UnboundedSlotEnqueuer &) = delete;

  ~UnboundedSlotEnqueuer() {
...
template <typename T> class UnboundedSlotDequeuer {
private:
  typedef uint64_t timestamp_t;
  constexpr static timestamp_t MAX_TIMESTAMP = ~((uint64_t)0);
  ...
  UnboundedSpscDequeuer<data_t> _spsc;
...
public:
  UnboundedSlotDequeuer(MPI_Aint dequeuer_rank, MPI_Aint self_rank,
                        MPI_Comm comm, MPI_Aint batch_size = 10)
      : _comm{comm}, _self_rank{self_rank}, _spsc{self_rank, comm},
  ...
  UnboundedSlotDequeuer(const UnboundedSlotDequeuer &) = delete;
  UnboundedSlotDequeuer &operator=(const UnboundedSlotDequeuer &) = delete;
  ~UnboundedSlotDequeuer() {
    MPI_Win_unlock_all(_min_timestamp_win);
    MPI_Win_free(&this->_min_timestamp_win);
