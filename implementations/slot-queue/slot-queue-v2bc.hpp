...
template <typename T> class SlotEnqueuerV2bc {
private:
...
public:
  SlotEnqueuerV2bc(MPI_Aint capacity, MPI_Aint dequeuer_rank,
  ...
  SlotEnqueuerV2bc(const SlotEnqueuerV2bc &) = delete;
  SlotEnqueuerV2bc &operator=(const SlotEnqueuerV2bc &) = delete;
...
template <typename T> class SlotDequeuerV2bc {
private:
  typedef uint64_t timestamp_t;
  constexpr static timestamp_t MAX_TIMESTAMP = ~((uint64_t)0);
...
public:
  SlotDequeuerV2bc(MPI_Aint capacity, MPI_Aint dequeuer_rank,
  ...
  SlotDequeuerV2bc(const SlotDequeuerV2bc &) = delete;
  SlotDequeuerV2bc &operator=(const SlotDequeuerV2bc &) = delete;
  ~SlotDequeuerV2bc() {
    MPI_Win_unlock_all(_counter_win);
    MPI_Win_unlock_all(_min_timestamp_win);
    MPI_Win_free(&this->_counter_win);
