...
int main(int argc, char **argv) {
  ...
  // parallel sort.
  std::sort(queues[BCL::rank()].begin().local(),
            queues[BCL::rank()].end().local());

  // Depending on problem size, it may be beneficial to do a "histogram sort"
  // instead. std::vector<int> counts = bucket_count(queues[BCL::rank()],
  // partition_size, BCL::rank()*partition_size);
