...
inline void
slotqueue_batch_one_queue_microbenchmark(unsigned long long number_of_elements,
                                          int batch_size, int iterations = 10) {
  int size;
  int rank;
...
inline void
ltqueue_batch_one_queue_microbenchmark(unsigned long long number_of_elements,
                                        int batch_size, int iterations = 10) {
  int size;
  int rank;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
...
inline void
fastqueue_batch_one_queue_microbenchmark(unsigned long long number_of_elements,
                                          int batch_size, int iterations = 10) {
  int size;
  int rank;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
